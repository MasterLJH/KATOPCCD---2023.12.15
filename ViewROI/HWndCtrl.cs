...
namespace ViewROI
{
    ...
    public class HWndCtrl
    {
        ...
        /// 是否显示图像点位信息
        /// </summary>
        public bool Image_Info_Display = false;
        /// <summary>
        /// 显示图像点位信息时的颜色
        /// </summary>
        public string Image_Info_Color = "magenta";
        ...
        private void ViewPort_HMouseMove(object sender, HMouseEventArgsWPF e)
        {
            ...
            Pixel_Col = Convert.ToInt32(e.Column);
            Pixel_Row = Convert.ToInt32(e.Row);
            ...
            if (viewPort.HalconWindow != null && Image_Info_Display)
            {
                

                if (Pixel_Col >= 0 && Pixel_Col < ImageWidth && Pixel_Row >= 0 && Pixel_Row < ImageHeight)
                {
                    int p = Pixel_Row * ImageWidth + Pixel_Col;
                    if (Pixel_Array_R != null && Pixel_Array_G != null && Pixel_Array_B != null)
                    {
                        if (Pixel_Array_R.Length > p)
                        {
                            Pixel_R = Pixel_Array_R[p];
                            Pixel_G = Pixel_Array_G[p];
                            Pixel_B = Pixel_Array_B[p];
                        }
                    }
                }
            }
        ...
        public void repaint(HalconDotNet.HWindow window)
        {
            ...
            if (Image_Info_Display)
            {
                int wrow, wcol, ww, wh;
                window.GetWindowExtents(out wrow, out wcol, out ww, out wh);
                set_display_font(viewPort.HalconID, 14, "mono", "false", "false");
                window.DispText("R:" + Pixel_R + " G:" + Pixel_G + " B:" + Pixel_B + "  Row:" + Pixel_Row + ",Col:" + Pixel_Col, "window", wh - 20, ww - 300, Image_Info_Color, new HTuple().TupleConcat("box"), new HTuple().TupleConcat("false"));
            }
        }
      ...
      HTuple hv_Bold, HTuple hv_Slant)
        ...
        //显示灰度值用
        byte[] Pixel_Array_R, Pixel_Array_G, Pixel_Array_B;
        int ImageWidth;
        int ImageHeight;
        /********************************************************************/
        /*                      GRAPHICSSTACK                               */
        ...
        public void addIconicVar(HObject obj)
        {
            ...
            if (obj is HImage)
            {
                //如果是HImage类型,就清除所有HObjectEntry和HMsgEntry
                clearList();
                clearMsgList();
                ...
                HTuple channels;
                HOperatorSet.CountChannels(obj, out channels);
                //版本V1.12 更新图像时获取图像宽高和像素色彩
                if (channels.I == 1)//单通道黑白
                {
                    HTuple P, Type, Width, Height;
                    HOperatorSet.GetImagePointer1(obj, out P, out Type, out Width, out Height);
                    byte[] Pixels = new byte[Width * Height];
                    Marshal.Copy(P.IP, Pixels, 0, Pixels.Length);
                    Pixel_Array_R = Pixels;
                    Pixel_Array_G = Pixels;
                    Pixel_Array_B = Pixels;
                ...
                else if (channels.I == 3)//三通道彩色
                {
                    HTuple Red, Green, Blue, Type, Width, Height;
                    HOperatorSet.GetImagePointer3(obj, out Red, out Green, out Blue, out Type, out Width, out Height);

                    Pixel_Array_R = new byte[Width * Height];
                    Marshal.Copy(Red.IP, Pixel_Array_R, 0, Pixel_Array_R.Length);

                    Pixel_Array_G = new byte[Width * Height];
                    Marshal.Copy(Green.IP, Pixel_Array_G, 0, Pixel_Array_G.Length);

                    Pixel_Array_B = new byte[Width * Height];
                    Marshal.Copy(Blue.IP, Pixel_Array_B, 0, Pixel_Array_B.Length);
